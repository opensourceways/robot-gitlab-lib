...
build(){
    ...
    git clone https://github.com/opensourceways/robot-gitlab-lib.git

    cp -r robot-gitlab-lib/template/. .

    rm -fr robot-gitlab-lib
...
generate robot codes for gitlab platform at current dir with
robot name of 'robot-gitlab-test' and
import path of 'github.com/opensourceways/robot-gitlab-test'.
